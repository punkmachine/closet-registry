...
import {
  ...
  pluginVersionListResponseSchema,
...
const pluginsRoutes: FastifyPluginAsyncZod = async (fastify) => {
  ...
  // Регистрируется ДО "/plugins/:slug/:version": find-my-way всё равно отдаёт приоритет
  // статическому сегменту над параметром, а "versions" не является валидным semver,
  // так что коллизии с реальной версией нет.
  fastify.get(
    "/plugins/:slug/versions",
    {
      schema: {
        ...
        response: {
          200: pluginVersionListResponseSchema,
    ...
    async (request, reply) => {
      ...
      const versions = await findActiveVersionsBySlug(orm, slug);
